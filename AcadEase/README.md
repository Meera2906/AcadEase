...
>
> 🎬 [DEMO-GUIDE.md](./DEMO-GUIDE.md) — the four-laptop run of show, and how to
> test the deployed site before presenting ·
...
- **Verify a certificate handed to you as a file** — staff drop a PDF, photo or
  scan into *Verify Certificate*; the QR is read, the issued record is checked
  against its signatures, **and the file itself is hashed and compared with the
  PDF we generated**. That second check is what catches a forged printout built
  around a genuine QR code — the record verifies, the file does not.
...
| **`faculty`** | Marks attendance, enters assessment marks and results, handles OD requests, verifies certificates handed to them, uploads study materials, issues circulars | **Own department** |
...
├── DEMO-GUIDE.md               ← four-laptop run of show + preflight testing
...
└── apps/
    ...
    │   ├── scripts/            export-keys.mjs (pin signing keys)
    │   │                       demo-preflight.mjs (check the deployed demo)
...
Manual walkthrough, per-role checklists, the run of show and a troubleshooting
table: **[DEMO-GUIDE.md](./DEMO-GUIDE.md)**.
